...
export function createFlightCommentator({
  ...
  return async function comment(payload) {
    ...
    const systemInstruction = [
      ...
      'ルート逸脱はFiled Route座標とActual Trackの両方がある場合だけ評価してください。Filed Route座標がなくても textualRoute があれば、計画経路文字列として説明に使ってください。',
      '事故・緊急事態・安全性は、このデータだけでは確認できません。事故が起きていないと断定しないでください。',
      '現在位置がある場合は、緯度経度、高度、対地速度、方位、上昇下降情報を必ず簡潔に説明してください。',
      'weather がある場合は出発地・到着地のMETAR観測として説明し、航空機現在位置の天候と誤認しないでください。',
      '位置更新停止、欠損、推定高度だけを事故の兆候と扱わないでください。',
      '出力は280〜450文字程度。見出しは「現在の状況」「運航・ルート」「天候」「安全に関する注意」の4つまで。',
    ].join('\n');
